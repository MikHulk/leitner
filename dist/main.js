...
var $author$project$Main$HomeView = {$: 'HomeView'};
var $author$project$Main$NewTime = function (a) {
	return {$: 'NewTime', a: a};
};
var $author$project$Main$Raw = {$: 'Raw'};
...
var $author$project$Main$MathFormula = {$: 'MathFormula'};
var $author$project$Main$parseCard = function (card) {
	return {
		correctAnswer: {
			contentType: function () {
				var _v0 = card.correctAnswer.contentType;
				if (_v0 === 'MathFormula') {
					return $author$project$Main$MathFormula;
				} else {
					return $author$project$Main$Raw;
				}
			}(),
			value: card.correctAnswer.value
		},
		nextReviewAt: $elm$time$Time$millisToPosix(card.nextReviewAt),
		question: card.question,
		rank: card.rank
	};
};
var $author$project$Main$init = function (cards) {
	return _Utils_Tuple2(
		{
			answerRevealed: false,
			cards: A2($elm$core$List$map, $author$project$Main$parseCard, cards),
			newQuestion: {answer: '', contentType: $author$project$Main$Raw, question: ''},
			now: $elm$time$Time$millisToPosix(0),
			view: $author$project$Main$HomeView
		},
...
var $author$project$Main$AddQuestionView = {$: 'AddQuestionView'};
var $author$project$Main$AnswerQuestionView = {$: 'AnswerQuestionView'};
var $author$project$Main$ManageQuestionsView = {$: 'ManageQuestionsView'};
...
var $author$project$Main$encodeCard = function (card) {
	return {
		...
		nextReviewAt: $elm$time$Time$posixToMillis(card.nextReviewAt),
		question: card.question,
		rank: card.rank
	};
};
...
var $author$project$Main$insertCard = F2(
	function (cards, newCard) {
		var go = F2(
			function (checked, remain) {
				go:
				while (true) {
					if (!remain.b) {
						return $elm$core$List$reverse(
							A2($elm$core$List$cons, newCard, checked));
					} else {
						var cur = remain.a;
						var rem = remain.b;
						var millisNewCard = $elm$time$Time$posixToMillis(newCard.nextReviewAt);
						var millisCurrent = $elm$time$Time$posixToMillis(cur.nextReviewAt);
						if (_Utils_cmp(millisCurrent, millisNewCard) > 0) {
							return _Utils_ap(
								$elm$core$List$reverse(checked),
								A2(
									$elm$core$List$cons,
									newCard,
									A2($elm$core$List$cons, cur, rem)));
						} else {
							var $temp$checked = A2($elm$core$List$cons, cur, checked),
								$temp$remain = rem;
		...
		return A2(go, _List_Nil, cards);
	});
var $elm$core$Basics$pow = _Basics_pow;
var $author$project$Main$oneDay = 864 * A2($elm$core$Basics$pow, 10, 5);
var $author$project$Main$fibo = function (n) {
	var go = F3(
		function (a, b, c) {
			go:
			while (true) {
...
var $author$project$Main$processCard = F3(
	function (now, card, answerIsGood) {
		var rank = answerIsGood ? ((card.rank >= 12) ? 12 : (card.rank + 1)) : 0;
		var timeOffset = $author$project$Main$oneDay * $author$project$Main$fibo(rank);
		var nextDate = $elm$time$Time$millisToPosix(
			$elm$time$Time$posixToMillis(now) + timeOffset);
		return _Utils_update(
			card,
			{nextReviewAt: nextDate, rank: rank});
	});
var $elm$json$Json$Encode$int = _Json_wrap;
var $author$project$Main$saveCards = _Platform_outgoingPort(
	'saveCards',
	$elm$json$Json$Encode$list(
		function ($) {
			return $elm$json$Json$Encode$object(
				_List_fromArray(
					[
						...
						_Utils_Tuple2(
						'nextReviewAt',
						$elm$json$Json$Encode$int($.nextReviewAt)),
						_Utils_Tuple2(
						'question',
						$elm$json$Json$Encode$string($.question)),
						_Utils_Tuple2(
						'rank',
						$elm$json$Json$Encode$int($.rank))
					]));
		}));
...
var $author$project$Main$userAction = F2(
	function (action, model) {
		switch (action.$) {
			case 'ChooseAddQuestion':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							newQuestion: {answer: '', contentType: $author$project$Main$Raw, question: ''},
							view: $author$project$Main$AddQuestionView
						}),
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'ChooseAnswerQuestion':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{answerRevealed: false, view: $author$project$Main$AnswerQuestionView}),
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'ChooseManageQuestions':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{answerRevealed: false, view: $author$project$Main$ManageQuestionsView}),
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'WantGoHome':
				return _Utils_Tuple2(
					_Utils_update(
					...
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'TypeNewQuestion':
				var question = action.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
			...
			case 'TypeNewAnswer':
				...
				return _Utils_Tuple2(
					...
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'ChooseAnswerContentType':
				var contentType = action.a;
				var newQuestion = model.newQuestion;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							newQuestion: _Utils_update(
								newQuestion,
			...
			case 'SubmitNewCard':
				var newCard = {
					correctAnswer: {contentType: model.newQuestion.contentType, value: model.newQuestion.answer},
					nextReviewAt: $elm$time$Time$millisToPosix(
						$author$project$Main$oneDay + $elm$time$Time$posixToMillis(model.now)),
					question: model.newQuestion.question,
					rank: 0
				};
				var cards = A2($author$project$Main$insertCard, model.cards, newCard);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
					...
					$author$project$Main$saveCards(
						A2($elm$core$List$map, $author$project$Main$encodeCard, cards)));
			case 'RevealAnswer':
				return _Utils_Tuple2(
					_Utils_update(
						model,
					...
					A2($elm$core$Task$perform, $author$project$Main$NewTime, $elm$time$Time$now));
			case 'ValidateAnswer':
				var isValid = action.a;
				var _v1 = $elm$core$List$head(model.cards);
				if (_v1.$ === 'Just') {
					var card = _v1.a;
					var cards = A2(
						$author$project$Main$insertCard,
						function () {
							var _v2 = $elm$core$List$tail(model.cards);
							if (_v2.$ === 'Just') {
								var something = _v2.a;
								return something;
							} else {
						...
						A3($author$project$Main$processCard, model.now, card, isValid));
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{answerRevealed: false, cards: cards}),
						$author$project$Main$saveCards(
							A2($elm$core$List$map, $author$project$Main$encodeCard, cards)));
				} else {
					return _Utils_Tuple2(
			...
			default:
				...
				var newCards = _Utils_ap(
					A2($elm$core$List$take, index, model.cards),
					A2($elm$core$List$drop, index + 1, model.cards));
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{cards: newCards}),
					$author$project$Main$saveCards(
						A2($elm$core$List$map, $author$project$Main$encodeCard, newCards)));
		}
	});
var $author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'NewTime') {
			...
			var action = msg.a;
			return A2($author$project$Main$userAction, action, model);
		}
	});
var $author$project$Main$ChooseAnswerContentType = function (a) {
	return {$: 'ChooseAnswerContentType', a: a};
};
var $author$project$Main$SubmitNewCard = {$: 'SubmitNewCard'};
var $author$project$Main$TypeNewAnswer = function (a) {
	return {$: 'TypeNewAnswer', a: a};
};
var $author$project$Main$TypeNewQuestion = function (a) {
	return {$: 'TypeNewQuestion', a: a};
};
var $author$project$Main$UserInteracts = function (a) {
	return {$: 'UserInteracts', a: a};
};
var $author$project$Main$WantGoHome = {$: 'WantGoHome'};
...
var $author$project$Main$shadow = $mdgriffith$elm_ui$Element$Border$shadow(
	{
		blur: 1.5,
		color: A4($mdgriffith$elm_ui$Element$rgba255, 153, 160, 152, 0.3),
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A3($mdgriffith$elm_ui$Element$rgb255, 221, 224, 228)),
					...
					{bottom: 2, left: 1, right: 2, top: 1}),
					$mdgriffith$elm_ui$Element$Border$color(
					A3($mdgriffith$elm_ui$Element$rgb255, 150, 150, 150)),
					$mdgriffith$elm_ui$Element$Font$color(
					A3($mdgriffith$elm_ui$Element$rgb255, 93, 95, 102)),
					$author$project$Main$shadow,
...
var $author$project$Main$addQuestionView = function (model) {
	return A2(
		...
		_List_fromArray(
			[
					...
					]),
				{
					...
					onChange: function (s) {
						return $author$project$Main$UserInteracts(
							$author$project$Main$TypeNewQuestion(s));
					},
					placeholder: $elm$core$Maybe$Just(
						A2(
							$mdgriffith$elm_ui$Element$Input$placeholder,
							_List_Nil,
							$mdgriffith$elm_ui$Element$text('enter your question here'))),
					spellcheck: true,
					...
					]),
				{
					...
					onChange: function (s) {
						return $author$project$Main$UserInteracts(
							$author$project$Main$TypeNewAnswer(s));
					},
					placeholder: $elm$core$Maybe$Just(
						A2(
							$mdgriffith$elm_ui$Element$Input$placeholder,
							_List_Nil,
							$mdgriffith$elm_ui$Element$text('enter the answer here'))),
					spellcheck: true,
					...
					]),
				{
					label: A2(
						$mdgriffith$elm_ui$Element$Input$labelLeft,
						_List_Nil,
						$mdgriffith$elm_ui$Element$text('answer type :')),
					onChange: A2($elm$core$Basics$composeL, $author$project$Main$UserInteracts, $author$project$Main$ChooseAnswerContentType),
					options: _List_fromArray(
						[
							A2(
							$mdgriffith$elm_ui$Element$Input$option,
							$author$project$Main$Raw,
							$mdgriffith$elm_ui$Element$text('raw')),
							A2(
							$mdgriffith$elm_ui$Element$Input$option,
							$author$project$Main$MathFormula,
							$mdgriffith$elm_ui$Element$text('latex'))
						]),
					selected: $elm$core$Maybe$Just(model.newQuestion.contentType)
				}),
				A2(
				$mdgriffith$elm_ui$Element$row,
				_List_Nil,
				_List_fromArray(
					[
						...
						$author$project$Main$UserInteracts($author$project$Main$SubmitNewCard),
						'Submit'),
						A2(
						$author$project$Main$button,
						$author$project$Main$UserInteracts($author$project$Main$WantGoHome),
						'End')
					]))
			]));
};
var $author$project$Main$RevealAnswer = {$: 'RevealAnswer'};
var $author$project$Main$ValidateAnswer = function (a) {
	return {$: 'ValidateAnswer', a: a};
};
var $author$project$Main$cardElement = function (content) {
	return A2(
		$mdgriffith$elm_ui$Element$el,
		_List_fromArray(
			[
	...
	function (now, cards) {
		if (cards.b) {
			...
			return (_Utils_cmp(
				...
				$elm$time$Time$posixToMillis(card.nextReviewAt)) > 0) ? $elm$core$Maybe$Just(card) : $elm$core$Maybe$Nothing;
		} else {
			return $elm$core$Maybe$Nothing;
		}
	});
var $author$project$Main$answerView = function (model) {
	var _v0 = A2($author$project$Main$selectCard, model.now, model.cards);
	if (_v0.$ === 'Nothing') {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$el,
					_List_Nil,
					$mdgriffith$elm_ui$Element$text('Nothing to review. Take a rest.')),
					A2(
					$mdgriffith$elm_ui$Element$el,
					_List_fromArray(
						[$mdgriffith$elm_ui$Element$centerX]),
		...
		return A2(
			...
			_List_fromArray(
				[
					$author$project$Main$cardElement(
					$mdgriffith$elm_ui$Element$text(card.question)),
					model.answerRevealed ? $author$project$Main$cardElement(
					function () {
						var _v1 = card.correctAnswer.contentType;
						if (_v1.$ === 'Raw') {
							return $mdgriffith$elm_ui$Element$text(card.correctAnswer.value);
						} else {
							return $mdgriffith$elm_ui$Element$html(
								A3(
									...
									_List_fromArray(
										[
											A2($elm$html$Html$Attributes$attribute, 'src', card.correctAnswer.value)
										]),
									_List_Nil));
						}
					}()) : A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 75, 112, 200)),
							$mdgriffith$elm_ui$Element$height(
							$mdgriffith$elm_ui$Element$px(35)),
							$mdgriffith$elm_ui$Element$width(
							$mdgriffith$elm_ui$Element$px(200)),
							...
							{bottom: 2, left: 1, right: 2, top: 1}),
							$mdgriffith$elm_ui$Element$Border$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 75, 112, 213)),
							$mdgriffith$elm_ui$Element$Font$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 250, 250, 255)),
							$author$project$Main$shadow,
							$mdgriffith$elm_ui$Element$padding(0),
							$mdgriffith$elm_ui$Element$centerX
						]),
					{
						label: A2(
							...
							_List_fromArray(
								[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
							$mdgriffith$elm_ui$Element$text('Reveal answer')),
						onPress: $elm$core$Maybe$Just(
							$author$project$Main$UserInteracts($author$project$Main$RevealAnswer))
					}),
					model.answerRevealed ? A2(
					$mdgriffith$elm_ui$Element$row,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$centerX,
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$Background$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 75, 200, 112)),
									$mdgriffith$elm_ui$Element$height(
									$mdgriffith$elm_ui$Element$px(35)),
									$mdgriffith$elm_ui$Element$width(
									$mdgriffith$elm_ui$Element$px(200)),
									...
									{bottom: 2, left: 1, right: 2, top: 1}),
									$mdgriffith$elm_ui$Element$Border$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 75, 213, 112)),
									$mdgriffith$elm_ui$Element$Font$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 250, 250, 255)),
									$author$project$Main$shadow,
									$mdgriffith$elm_ui$Element$padding(0),
								...
								]),
							{
								label: A2(
									...
									_List_fromArray(
										[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
									$mdgriffith$elm_ui$Element$text('You knew it')),
								onPress: $elm$core$Maybe$Just(
									$author$project$Main$UserInteracts(
										$author$project$Main$ValidateAnswer(true)))
							}),
							A2(
							$mdgriffith$elm_ui$Element$Input$button,
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$Background$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 200, 50, 50)),
									$mdgriffith$elm_ui$Element$height(
									$mdgriffith$elm_ui$Element$px(35)),
									$mdgriffith$elm_ui$Element$width(
									$mdgriffith$elm_ui$Element$px(200)),
									...
									{bottom: 2, left: 1, right: 2, top: 1}),
									$mdgriffith$elm_ui$Element$Border$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 213, 75, 112)),
									$mdgriffith$elm_ui$Element$Font$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 250, 250, 255)),
									$author$project$Main$shadow,
									$mdgriffith$elm_ui$Element$padding(0),
								...
								]),
							{
								label: A2(
									...
									_List_fromArray(
										[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
									$mdgriffith$elm_ui$Element$text('You didn\'t know')),
								onPress: $elm$core$Maybe$Just(
									$author$project$Main$UserInteracts(
										$author$project$Main$ValidateAnswer(false)))
							})
						])) : $mdgriffith$elm_ui$Element$none,
					A2(
					$mdgriffith$elm_ui$Element$row,
...
var $author$project$Main$formatDate = function (time) {
	return $elm$core$String$concat(
		_List_fromArray(
			[
	...
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$centerX,
				$mdgriffith$elm_ui$Element$Background$color(
				A4($mdgriffith$elm_ui$Element$rgba255, 230, 230, 230, 0.8)),
				$mdgriffith$elm_ui$Element$Background$gradient(
				{
					angle: 0,
					steps: _List_fromArray(
						[
							A4($mdgriffith$elm_ui$Element$rgba255, 180, 205, 223, 1),
							A4($mdgriffith$elm_ui$Element$rgba255, 196, 208, 232, 0.8)
						])
				}),
				$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
				$mdgriffith$elm_ui$Element$height(
				...
				{
					blur: 2.0,
					color: A4($mdgriffith$elm_ui$Element$rgba255, 200, 200, 200, 0.5),
					offset: _Utils_Tuple2(0.0, 1.0),
					size: 2.0
				}),
				$mdgriffith$elm_ui$Element$Font$size(35),
				$mdgriffith$elm_ui$Element$Font$color(
				A3($mdgriffith$elm_ui$Element$rgb255, 62, 65, 60))
			]),
		A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
			$mdgriffith$elm_ui$Element$text(
				$author$project$Main$formatDate(model.now))));
};
var $author$project$Main$ChooseAddQuestion = {$: 'ChooseAddQuestion'};
var $author$project$Main$ChooseAnswerQuestion = {$: 'ChooseAnswerQuestion'};
var $author$project$Main$ChooseManageQuestions = {$: 'ChooseManageQuestions'};
var $author$project$Main$homeView = function (_v0) {
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$text('What do you want ?'),
				A2(
				$author$project$Main$button,
				$author$project$Main$UserInteracts($author$project$Main$ChooseAddQuestion),
				'Add Questions'),
				A2(
				$author$project$Main$button,
				$author$project$Main$UserInteracts($author$project$Main$ChooseAnswerQuestion),
				'Answer Questions'),
				A2(
				$author$project$Main$button,
				$author$project$Main$UserInteracts($author$project$Main$ChooseManageQuestions),
				'Manage Questions')
			]));
};
...
var $author$project$Main$manageQuestionView = function (model) {
	var headerBgColor = $mdgriffith$elm_ui$Element$Background$color(
		A4($mdgriffith$elm_ui$Element$rgba255, 128, 157, 193, 0.5));
			...
			[
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$centerX,
						$mdgriffith$elm_ui$Element$Background$color(
						A3($mdgriffith$elm_ui$Element$rgb255, 250, 255, 250))
					]),
				{
						...
						[
							...
							view: function (card) {
								return A2(
									...
									cellStyle,
									$mdgriffith$elm_ui$Element$text(
										A2($elm$core$String$left, 40, card.question) + '...'));
							},
							...
							header: A2(
								$mdgriffith$elm_ui$Element$el,
								headerStyle,
								$mdgriffith$elm_ui$Element$text('rank')),
							view: function (card) {
								return A2(
									$mdgriffith$elm_ui$Element$el,
									cellStyle,
									$mdgriffith$elm_ui$Element$text(
										$elm$core$String$fromInt(card.rank)));
							},
							...
							header: A2(
								$mdgriffith$elm_ui$Element$el,
								headerStyle,
								$mdgriffith$elm_ui$Element$text('next review')),
							view: function (card) {
								return A2(
									$mdgriffith$elm_ui$Element$el,
									cellStyle,
									$mdgriffith$elm_ui$Element$text(
										$author$project$Main$formatDate(card.nextReviewAt)));
							},
							width: $mdgriffith$elm_ui$Element$px(200)
						},
							{
							header: A2(
								$mdgriffith$elm_ui$Element$el,
								_List_fromArray(
									[
										headerBgColor,
										$mdgriffith$elm_ui$Element$Font$color(
										A3($mdgriffith$elm_ui$Element$rgb255, 240, 255, 250)),
										$mdgriffith$elm_ui$Element$centerY,
										$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill)
									]),
								$mdgriffith$elm_ui$Element$text(' ')),
									...
									A2(
										...
										_List_fromArray(
											[
												$mdgriffith$elm_ui$Element$centerY,
												$mdgriffith$elm_ui$Element$Background$color(
												A3($mdgriffith$elm_ui$Element$rgb255, 230, 100, 100)),
												$mdgriffith$elm_ui$Element$Font$color(
												A3($mdgriffith$elm_ui$Element$rgb255, 230, 200, 200)),
												$mdgriffith$elm_ui$Element$paddingEach(
												{bottom: 4, left: 5, right: 5, top: 2}),
												$mdgriffith$elm_ui$Element$centerX
											]),
										{
											label: A2(
											...
											onPress: $elm$core$Maybe$Just(
												$author$project$Main$UserInteracts(
													$author$project$Main$RemoveCard(card.index)))
										}));
							},
					...
					data: A2(
						...
						F2(
							function (i, card) {
								return {index: i, nextReviewAt: card.nextReviewAt, question: card.question, rank: card.rank};
							}),
						model.cards)
				}),
				A2(
...
var $author$project$Main$view = function (model) {
	var mainView = function () {
		...
		switch (_v0.$) {
			...
			case 'AnswerQuestionView':
				return $author$project$Main$answerView(model);
			case 'AddQuestionView':
				return $author$project$Main$addQuestionView(model);
			default:
				return $author$project$Main$manageQuestionView(model);
		}
	}();
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$Background$color(
				A3($mdgriffith$elm_ui$Element$rgb255, 203, 200, 202)),
				$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
				$mdgriffith$elm_ui$Element$Font$color(
				A3($mdgriffith$elm_ui$Element$rgb255, 60, 90, 70))
			]),
		A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$centerX,
					$mdgriffith$elm_ui$Element$Background$color(
					A3($mdgriffith$elm_ui$Element$rgb255, 245, 245, 240)),
					$mdgriffith$elm_ui$Element$width(
								...
								return A2(
									...
									function (correctAnswer) {
										return $elm$json$Json$Decode$succeed(
											{correctAnswer: correctAnswer, nextReviewAt: nextReviewAt, question: question, rank: rank});
									},
									A2(
										$elm$json$Json$Decode$field,
										'correctAnswer',
										A2(
											...
											function (value) {
												return A2(
													...
													A2($elm$json$Json$Decode$field, 'contentType', $elm$json$Json$Decode$string));
											},
											A2($elm$json$Json$Decode$field, 'value', $elm$json$Json$Decode$string))));
							},
							A2($elm$json$Json$Decode$field, 'nextReviewAt', $elm$json$Json$Decode$int));
					},
					A2($elm$json$Json$Decode$field, 'question', $elm$json$Json$Decode$string));
			},
			A2($elm$json$Json$Decode$field, 'rank', $elm$json$Json$Decode$int))))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{},"unions":{"Main.Msg":{"args":[],"tags":{"NewTime":["Time.Posix"],"UserInteracts":["Main.UserAction"]}},"Time.Posix":{"args":[],"tags":{"Posix":["Basics.Int"]}},"Main.UserAction":{"args":[],"tags":{"ChooseAddQuestion":[],"ChooseAnswerQuestion":[],"ChooseManageQuestions":[],"WantGoHome":[],"TypeNewQuestion":["String.String"],"TypeNewAnswer":["String.String"],"ChooseAnswerContentType":["Main.Content"],"SubmitNewCard":[],"RevealAnswer":[],"ValidateAnswer":["Basics.Bool"],"RemoveCard":["Basics.Int"]}},"Basics.Bool":{"args":[],"tags":{"True":[],"False":[]}},"Main.Content":{"args":[],"tags":{"Raw":[],"MathFormula":[]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"String.String":{"args":[],"tags":{"String":[]}}}}})}});}(this));